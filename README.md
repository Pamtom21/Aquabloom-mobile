...
| Comando                                          | Uso                                   |
| ------------------------------------------------ | ------------------------------------- |
| npm run web                                      | Navegación y estados en navegador     |
| npm start                                        | Servidor para development build       |
| npm run android                                  | Abrir development build instalado     |
| npm run android:build                            | Compilación local con Android SDK/JDK |
| npm run check                                    | Formato, lint, TypeScript y pruebas   |
| npm run export:android                           | Bundle JS Android; no genera APK      |
| npm run doctor                                   | Diagnóstico Expo                      |
| npm run api:generate -- ./contracts/openapi.json | Tipos desde el contrato real          |
...
Iniciar sesión en EAS y ejecutar `npx eas-cli@latest init` seleccionando el proyecto del equipo para obtener projectId real. Confirmar el identificador provisional `com.aquabloom.mobile` antes de distribuir y configurar las variables públicas en EAS.
...
Obtener /openapi.json desde FastAPI, guardar en contracts/openapi.json y generar tipos. No se inventó un contrato. Acordar paginación, rutas, GeoJSON, errores, roles y semillas con Web/API.
